...
This recipe references the [_AWS PCS VPC and subnet requirements and considerations_](https://docs.aws.amazon.com/pcs/latest/userguide/working-with_networking_vpc-requirements.html) and [_Access AWS Parallel Computing Service using an interface endpoint (AWS PrivateLink)_](https://docs.aws.amazon.com/pcs/latest/userguide/vpc-interface-endpoints.html) sections of the AWS PCS User Guide.
...
- [`pcs-private-networking.yaml`](assets/pcs-private-networking.yaml)- Creates networking prerequisites (VPC, subnet, VPC endpoints) for your PCS controller and attached nodes.
- [`pcs-sharedstorage-sg.yaml`](assets/pcs-sharedstorage-sg.yaml) - Creates a security group for your EFS, FSx for Lustre, or FSx for NetApp ONTAP shared storage to be used by the cluster.
- [`pcs-lt.yaml`](assets/pcs-lt-efs-fsxl.yaml) - Creates a launch template for login node groups and compute node groups, with a shared home (EFS) and high-speed storage filesystem (FSx for Lustre or FSx for NetApp ONTAP).
- [`pcs-private-cluster.yaml`](assets/pcs-private-cluster.yaml) - Creates a launch template for login node groups and compute node groups, with a shared home (EFS) and high-speed storage filesystem (FSx for Lustre or FSx for NetApp ONTAP).
